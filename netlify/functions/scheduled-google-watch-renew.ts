...
export default async () => {
  ...
  try {
    ...
    if (result.action === "skipped") {
      ...
      return new Response(
        JSON.stringify({ status: "ok", action: "skipped", expiresInMs: result.expiresInMs, durationMs }),
        { status: 200 },
      );
    }
    ...
    return new Response(
      JSON.stringify({ status: "ok", action: "registered", expiresInMs: result.expiresInMs, durationMs }),
      { status: 200 },
    );
  } catch (error) {
    const durationMs = Date.now() - started;
    ...
    return new Response(
      JSON.stringify({ status: "failed", error: "watch_auto_renew_failed", durationMs }),
      { status: 500 },
    );
  }
};
