...
export function MonthBoard({
  ...
  // Diagnostic: ?debugLabels=1 annotates every Month-view LA# label element
  // with a color-coded outline + a floating badge showing computed font-size,
  // and dumps console.table of all computed text metrics. Without the param,
  // this effect short-circuits and does nothing.
  useEffect(() => {
    if (typeof window === "undefined") return undefined;
    if (new URLSearchParams(window.location.search).get("debugLabels") !== "1") {
      return undefined;
    }

    const SELECTORS = [
      { sel: ".month-row-bar", color: "#facc15", label: "row-bar" },
      { sel: ".month-row-bar-label", color: "#22d3ee", label: "row-bar-label" },
      { sel: ".month-day-chip", color: "#f0abfc", label: "day-chip" },
    ];

    const cleanups: Array<() => void> = [];
    const trackPairs: Array<{ el: HTMLElement; badge: HTMLElement }> = [];
    const tableRows: Array<Record<string, string>> = [];

    for (const { sel, color, label } of SELECTORS) {
      ...
      elements.forEach((el) => {
        ...
        badge.textContent = `${label}:${cs.fontSize}`;
        badge.style.cssText = [
          "position: fixed",
          `top: ${Math.max(0, Math.round(rect.top - 11))}px`,
    ...
    const onUpdate = () => {
      for (const { el, badge } of trackPairs) {
        const r = el.getBoundingClientRect();
        badge.style.top = `${Math.max(0, Math.round(r.top - 11))}px`;
    ...
    if (tableRows.length > 0) {
      console.group(`[debugLabels] Month view labels (${tableRows.length})`);
