...
describe("isPayloadAnImprovement", () => {
  ...
  it("does not replace route-target list payload with a richer non-target week payload", () => {
    ...
    expect(isPayloadAnImprovement({ incoming, current, target: routeTarget })).toBe(false);
  });

  it("does not replace route-target month payload with a richer non-target month payload", () => {
